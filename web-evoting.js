...
const evoting = require('../ethereum/evoting');
let accounts = []
...
router.get("/addPasien", async (req, res, next) => {
  try{
...
router.get("/getpasien_pasien", async (req, res, next) => {
  try{
    const pasienData = await evoting.methods.getpasien_pasien().call({ from: accounts[0] })
    res.json({
      pasienData: pasienData
    });
  }catch (e){
